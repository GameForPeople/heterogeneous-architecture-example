...
void Framework::Run()
{
	/*
		...
		1. 이 예제에서는 총 3개의 스레드가 존재합니다.
		 - 메인 스레드        : main문 스레드 ( IOCP일때를 생각하면, Recv가 이루어지는 I/O 스레드라고 생각해도 좋을 것 같습니다! )
		...
		2. 핵심은 TaskManager 클래스에서, PushTask 함수를 통해, Task를 비동기적으로 다른 Context로 넘기는 것입니다.
			- A Context에서 B Context를 향해 PushTask로 Task를 넘길 때,
				- B객체의 MS-Safe Queue(PPL)에 Push해서, MP에 안전하게 처리하고,
				- 이후, B 객체가 자신이 유효한 Context에서 Pop함으로서, SC에 안전하게 처리합니다.
			- 명확하게 이해가 어려울 수 있습니다. 아래 예제를 확인하시는 것을 추천드립니다.
...
#pragma region 유저생성_혹은_로그인_예제
	//	먼저 유저 생성 혹은, 로그인과 관련된 예제입니다.
	//	 - 간단한 예제를 목표로하다 보니, 따로 DB를 붙이지 않으니; 항상 유저 생성과 로그인이 동시에 이루어진다고 가정합니다.
			...
			[ id ]()
			{
				...
				// 예외 처리
				{
						...
						; currentUserCount >= UserManager::MAX_USER_NUM )
					{
						...
						PRINT_LOG( "로그인 실패! 이미 최대 동접! // 현재 동접 수 : " + std::to_string( currentUserCount ) );
						return;
					}
					...
					{
						// 이미 접속해있는 Id입니다!
						PRINT_LOG( "로그인 실패! 이미 접속한 ID // id : " + id );
						return;
					}
				}
	...
	//		- ThreadID를 확인해보면 더 명확하게 인지할 수 있습니다.
...
#pragma region 방_생성_예제
	...
	//	 - 따라서 코드를 바로 작성하겠습니다. 아래의 코드를 한번 봐주세요!
	{
		...
		UserUnit* userUnit = UserManager::GetInstance().m_userCont[ "NetmarbleNeo" ]; // 위에서 만든 유저입니다.
		...
		RoomManager::GetInstance().PushTask( 
			[ roomName, userUnit ]()
			{
					...
					; currentRoomCount >= RoomManager::MAX_ROOM_NUM )
				{
					...
					PRINT_LOG( "방 생성 실패! 이미 최대 방 개수! // userId : " + userUnit->m_id + ",현재 방 수 : " + std::to_string( currentRoomCount ) );
					return;
				}
				...
				{
					// 이미 생성되어있는 이름입니다!
					PRINT_LOG( "방 생성 실패! 이미 만들어져있는 이름입니다. // userId : " + userUnit->m_id + ", name : " + roomName );
					return;
				}
				...
				PRINT_LOG( "방 생성 성공! // userId : " + userUnit->m_id + ", name : " + roomName );
		...
		UserManager::GetInstance().PushTask(
			[ userUnit, chatType, chatMessage ]()
			{
				...
				// 예외 처리
				{
					...
					{
						PRINT_LOG( "뭐여 로비에 없는데, 로비 채팅을 요청했습니다. // 유저 이름 : " + userUnit->m_id );
					}
					...
					{
						PRINT_LOG( "뭐여 방에 없는데, 방 채팅을 요청했습니다. // 유저 이름 : " + userUnit->m_id );
					}
				}
				...
				if ( chatType == ChatType::Lobby )
				{
					...
					PRINT_LOG( "로비 채팅 성공! // userId : " + userUnit->m_id + ", msg : " + chatMessage );
				}
				// 혹은 방 채팅을 처리합니다.
				else if ( chatType == ChatType::Room )
				{
					...
					RoomManager::GetInstance().PushTask(
							...
							chatMessage ]()
						{
							...
							PRINT_LOG( "방 채팅 성공! // userId : " + userUnit->m_id + ", msg : " + chatMessage );
						} );
				}
			}
		);
...
#pragma region 예제를_마치며
	/*
		...
		사실 위 예제로 구현된 heterogeneous 방식은 위에서 말씀드린 문제 외에도, 여러 문제를 갖고 있습니다.
			- 각 Manager 별로 Thread를 소유하고 있어, Thread별로 부하 분산이 되지 않는 이슈
		...
		저 원래 꼰대인건 알았는데 이번에 멘토링하면서 느끼는데 너무 꼰대네요....젊꼰은 답도 없다는데.....죄송합니다...
